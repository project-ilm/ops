#!/usr/bin/env bash
# fix_3d_explorer.sh — overwrite explore/index.html with the corrected explorer (calm spin + labels).
# Robust: no anchor matching. One-file incremental patch, loud PR. context: https://ilm.codes/context/
set -euo pipefail
source "$(dirname "$0")/lib.sh"
D="$(ilm_repo ilm.codes)"
ilm_branch "$D" ai/fix-3d-spin-labels
cat > "$D/explore/index.html" <<'HTML'
...
(function(){
 ...
 function label(txt){var cv=document.createElement("canvas");cv.width=128;cv.height=64;
   var x=cv.getContext("2d");x.fillStyle="#eef1ff";x.font="bold 44px monospace";x.fillText(txt,6,46);
   var sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(cv),transparent:true}));
   sp.scale.set(58,29,1);return sp;}
 ...
 function Controls(camera,dom,tgt){
  ...
  function apply(){ camera.position.set(
    ...
    camera.lookAt(tgt); }
  this.tick=function(){ if(self.autoRotate && !drag){ az+=0.00042; apply(); } };
  this.apply=apply;
...
git -C "$D" commit -q -m "fix(explore): calm auto-rotate + persistent family labels + brighter L0-L9 (full-file, robust)"
ilm_pr "$D" ai/fix-3d-spin-labels "fix(explore): calmer spin + readable labels" "Auto-rotate ~4x slower; persistent coloured family-cluster labels; brighter L0-L9. Full-file replacement so it applies regardless of current state."
